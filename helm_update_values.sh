...
if [ "${1}" == "" ]; then
    echo "Usage: ./helm_update_values.sh <YAML_OVERRIDE_VALUES_JSON> [GIT_COMMIT_MESSAGE] [GIT_REPO_TOKEN] [GIT_REPO_SLUG]"
fi
...
if [ "${GIT_COMMIT_MESSAGE}" != "" ] && [ "${GIT_REPO_TOKEN}" != "" ] && [ "${GIT_REPO_SLUG}" != "" ]; then
    ...
    ! git clone --depth 1 "https://github.com/${GIT_REPO_SLUG}.git" "${TEMPDIR}" \
    ...
    if ! git diff --shortstat --exit-code "environments/${K8S_ENVIRONMENT_NAME}/values.auto-updated.yaml"; then
        ...
        ! (
            ...
            git push https://${GIT_REPO_TOKEN}'@'github.com/${GIT_REPO_SLUG}.git master
